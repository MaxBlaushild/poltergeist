...
// isPuzzleSolved checks if the puzzle is already in gold (success) state
...
func (c *utilityClosetPuzzleClient) handleButtonPress(
	...
	case 0: // Off (Gray) -> Green
		puzzle.SetButtonCurrentHue(slot, 2) // Green
	...
	case 2: // Green -> Red
		...
		puzzle.SetButtonCurrentHue(slot, 4) // Red
...
// Success condition: Gray (0) → Blue (1) → Green (2) → White (3) → Red (4) → Purple (5)
...
// setAllLightsToGold sets all lights to gold (success state)
func (c *utilityClosetPuzzleClient) setAllLightsToGold(puzzle *models.UtilityClosetPuzzle) {
	for i := 0; i < 6; i++ {
