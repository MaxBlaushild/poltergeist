...
ALTER TABLE quest_nodes
  ...
  CHECK (
    (
      ...
      (CASE WHEN exposition_id IS NOT NULL THEN 1 ELSE 0 END) +
      (CASE WHEN fetch_character_id IS NOT NULL THEN 1 ELSE 0 END) +
      (CASE WHEN NULLIF(BTRIM(story_flag_key), '') IS NOT NULL THEN 1 ELSE 0 END)
    ) = 1
  );
