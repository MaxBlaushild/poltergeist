...
func (h *characterHandler) FindByID(ctx context.Context, id uuid.UUID) (*models.Character, error) {
	...
	if err := h.db.WithContext(ctx).Preload("MovementPattern").Preload("Location").First(&character, "id = ?", id).Error; err != nil {
...
func (h *characterHandler) FindAll(ctx context.Context) ([]*models.Character, error) {
	...
	if err := h.db.WithContext(ctx).Preload("MovementPattern").Preload("Location").Find(&characters).Error; err != nil {
		return nil, err
	}
...
func (h *characterHandler) FindByLocationID(ctx context.Context, locationID uuid.UUID) ([]*models.Character, error) {
	var characters []*models.Character
	if err := h.db.WithContext(ctx).Preload("MovementPattern").Preload("Location").Where("location_id = ?", locationID).Find(&characters).Error; err != nil {
		return nil, err
	}
	return characters, nil
}
