package db

import (
	"context"

	"github.com/MaxBlaushild/poltergeist/pkg/models"
	"github.com/google/uuid"
	"gorm.io/gorm"
)

func defaultCharacterGenreID(
	ctx context.Context,
	database *gorm.DB,
) (uuid.UUID, error) {
	var genre models.ZoneGenre
	if err := database.WithContext(ctx).
		Select("id").
		Where("LOWER(name) = LOWER(?)", models.DefaultZoneGenreNameFantasy).
		Order("sort_order ASC").
		Order("created_at ASC").
		First(&genre).Error; err != nil {
		return uuid.Nil, err
	}
	return genre.ID, nil
}

func resolveCharacterGenreID(
	ctx context.Context,
	database *gorm.DB,
	character *models.Character,
) (uuid.UUID, error) {
	if character != nil && character.GenreID != uuid.Nil {
		return character.GenreID, nil
	}
	return defaultCharacterGenreID(ctx, database)
}

func resolveCharacterTemplateGenreID(
	ctx context.Context,
	database *gorm.DB,
	template *models.CharacterTemplate,
) (uuid.UUID, error) {
	if template != nil && template.GenreID != uuid.Nil {
		return template.GenreID, nil
	}
	return defaultCharacterGenreID(ctx, database)
}
