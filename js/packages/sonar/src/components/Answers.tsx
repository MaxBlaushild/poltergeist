...
export const Answers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSurveysAndAnswers = async () => {
      try {
        const submissions = await apiClient.get<Submission[]>('/sonar/surveys/submissions');
  ...
  const handleRemoveActivityFilter = (filterToRemove: string): void => {
    setActivityFilters(activityFilters.filter(filter => filter !== filterToRemove));
  };
  ...
  submissions.forEach(submission =>
    submission.answers.forEach(answer => {
        if (nameFilters.some(filter =>
            ...
            if (dupeMap[answer.activityId] && dupeMap[answer.activityId][submission.userId]) {
      ...
      <h1>Your Survey Answers</h1>
          <div>
            ...
            {nameFilters.map((filter, index) => (
              <div key={index}>
                {filter} <button onClick={() => handleRemoveNameFilter(filter)}>Remove</button>
              </div>
            ))}
          </div>
          <div>
            ...
            {activityFilters.map((filter, index) => (
              <div key={index}>
                {filter} <button onClick={() => handleRemoveActivityFilter(filter)}>Remove</button>
              </div>
            ))}
          </div>
      {submissions.length > 0 ? (
                <ul>
          {submissions.map((submission, index) => submission.answers.filter((answer) => answer.down && (![...nameFilters, ...activityFilters].length || answers[answer.id])).map((answer, index) => (
            <li key={index}>
              <strong>User:</strong> {submission.user.name} <strong>Activity:</strong> {answer.activity.title}
            </li>
          )))}
        </ul>
