...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const auth = await reefApi.login(email, password);
      login(auth);
      navigate('/account');
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-sm space-y-6">
      <h1 className="font-display text-2xl font-bold text-reef-ink">Log in</h1>
      ...
      <p className="text-center text-sm text-reef-ink/60">
        No account?{' '}
        <Link to="/signup" className="font-medium text-reef-teal underline underline-offset-2">
          Sign up
        </Link>
      </p>
    </div>
