...
import type { GMLibraryItem, GMLibraryQuizQuestion } from '../../gmApi';
import { ApiError } from '../../api';
import { Card } from './GameSection';

// Content tab: choose which of the shared story's items and quiz questions
// are included in this Toast. (Which characters are "in" is implicit now —
// see the Invites tab: a character is in play once someone's been invited to
// it.) Toggling off something that's still in active use is blocked (409)
// rather than silently allowed — the error names exactly what's blocking it.
export const ContentSection = () => {
  return (
    <div className="flex flex-col gap-4">
      <p className="text-bone/50 text-sm">
        Trim items and quiz questions to fit your event. Everything starts included; toggling something
        off that's still assigned to a player (or already answered, for quiz questions) is blocked —
        reassign it first.
      </p>
      <ItemsPanel />
      <QuizPanel />
